...
class SaleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('petugas-page.sale', compact('data'));
    }
    ...
    public function productSale(Request $request)
    {
        $data = Product::all();
        // dd($data);
        return view('petugas-page.create-sale', compact('data'));
    }
    ...
    public function dashboard()
{
    ...
    return view('petugas-page.dashboard', compact('todaySalesCount'));
}
    ...
    public function checkout(Request $request)
    {
        ...
        return view('petugas-page.checkout', compact('checkout'));
    }
    ...
    public function paymentTransaction(Request $request)
    {
        ...
        if ($request->is_member == 1) {
            ...
            $member = DB::table('members')->where('no_telp', $request->phone_number)->first();
            // dd($member);
            ...
            return view('petugas-page.member', compact(
                'cart_items',
                'sub_total',
                'amount_paid',
                'member_name',
    ...
    public function memberTransaction(Request $request)
    {
        ...
        if ($request->use_point > 0) {
            DB::table('members')->where('id', $memberId)->update(['poin' => 0]);

            $point_use = $request->total_point;
            $total = max($sub_total - $point_use, 0);
            $change = ($amount_paid - ($sub_total - $point_use));
        } else {
            DB::table('members')->where('id', $memberId)->update(['poin' => $request->total_point]);
    ...
    public function showReceipt($id)
    {
        ...
        return view('petugas-page.receipt', compact('saleData'));
    }
    ...
    public function printPDF(Request $request, $id)
    {
        ...
        $pdf = PDF::loadView('petugas-page.invoice', compact('saleData'));
        ...
        $data = DB::table('sales as s')
            ...
            ->when($keyword, function ($query, $keyword) {
                return $query->where(function ($subQuery) use ($keyword) {
                    $subQuery->where('m.name', 'LIKE', "%$keyword%")
                             ->orWhere('s.date', 'LIKE', "%$keyword%");
                });
            })
            ->when($filter, function ($query, $filter) {
                $now = Carbon::now();
                if ($filter === 'hari') {
                    return $query->whereDate('s.created_at', $now->toDateString());
                } elseif ($filter === 'minggu') {
                    return $query->whereBetween('s.created_at', [$now->startOfWeek(), $now->endOfWeek()]);
                } elseif ($filter === 'bulan') {
                    return $query->whereMonth('s.created_at', $now->month)
                                 ->whereYear('s.created_at', $now->year);
                } elseif ($filter === 'tahun') {
                    return $query->whereYear('s.created_at', $now->year);
                }
            })
            ->orderByDesc('s.created_at')
            ->simplePaginate(6);
    
        return view('admin-page.sale', compact('data'));
    }
...
{
    $filter = $request->query('filter', null);
    return Excel::download(new SalesExport($filter), 'admin-sale.xlsx');
}
...
public function adminPage()
{
    ...
    $weeklySales = Sale::selectRaw("DATE(created_at) as date, COUNT(*) as total_sales, SUM(total) as total_revenue")
        ->whereBetween('created_at', [$startOfWeek, $endOfWeek])
        ->groupBy('date') // Add all non-aggregated columns to GROUP BY
        ->orderBy('date', 'ASC')
        ->get();
    
    // Format data for bar chart
    $dates = $weeklySales->pluck('date')->map(fn($date) => Carbon::parse($date)->format('D, d M'))->toArray();
    $salesCountChart = $weeklySales->pluck('total_sales')->toArray();
    $revenueChart = $weeklySales->pluck('total_revenue')->toArray();
    
    // Top selling products
    $productSales = DB::table('sale_details')
        ->join('products', 'sale_details.product_id', '=', 'products.id')
        ->selectRaw('products.nama_produk as product_name, SUM(sale_details.product_qty) as total_sold')
        ->groupBy('products.nama_produk') // Use the actual column name for grouping
    ...
    // Recent transactions
    $recentTransactions = Sale::with('member')
